...
qsub $scriptsFolder/run_atlasWerks.bash
...
cd /stage/sharonxx/proj/mskcc/NCAT$NUM/rcct/shap
...
#for i in 00 10 20 30 40 50 60 70 80 90 ; do  DistanceMap  lung-bin-cinePhase$i.mhd   lung-bin-cinePhase$i.dis.mhd  10; done


./writeSeeds_NCAT.m  # using  padded image to mark the seeds points

#preprocessing for the correpondence program, generate smoothed distance map
#ShapeWorksGroom   groom.params  antialias fastmarching blur   # extremely slow!!!! can be speed up by first crop the images
...
#7. segment CBCT
mkdir /stage/sharonxx/proj/mskcc/NCAT$NUM/cbct/segmentation/xyz


# bash scripts
  qsub ~sharonxx/scripts/SCDS-cbct/NCAT/meshDeform-CBCT.bash 2
