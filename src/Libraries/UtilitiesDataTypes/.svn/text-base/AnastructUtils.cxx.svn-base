...
void
AnastructUtils
::anastructToSurface(const Anastruct& anastruct, Surface& surface)
{
  // this clears the tiles in the contour tiler.
  // it is required because the tiler saves information
  // in global variables
  clearTiles();
  
  //
  // massage data into form the tiler likes i.e. a float***
  // i.e. point[contourIndex][pointIndex][0/1/2 for x/y/z]
  // 
  const int numContours = anastruct.contours.size();
  int *pointsPerContour = new int[numContours];
  typedef float** floatdptr;
  typedef float* floatptr;
  float ***points = new floatdptr[numContours];
  int contourIndex = 0;
  for (contourIndex = 0; contourIndex < numContours; ++contourIndex)
    {
      int numPoints = anastruct.contours[contourIndex].vertices.size();
      pointsPerContour[contourIndex] = numPoints;

      points[contourIndex] = new floatptr[numPoints];
      for (int pointIndex = 0; pointIndex < numPoints; ++pointIndex)
	{
	  points[contourIndex][pointIndex] = new float[3];
	  points[contourIndex][pointIndex][0] = 
	    anastruct.contours[contourIndex].vertices[pointIndex].x;
	  points[contourIndex][pointIndex][1] = 
	    anastruct.contours[contourIndex].vertices[pointIndex].y;
	  points[contourIndex][pointIndex][2] = 
	    anastruct.contours[contourIndex].vertices[pointIndex].z;
	}
    }      

  //
  // run the tiler and get tiles
  //
  setContours(numContours, pointsPerContour, points);
  //
  // default options:
  //   - outer contours are drawn clockwise, inner drawn CCW
  //   - omit horizontal tiles
  //   - use default perturbation distance
  //   - produce caps
  //   - no tiles representing normals
  //
  // CTI_PERT_DIST
  //
  cti_flags_t tilerFlags = CTI_CW | CTI_CAPS | CTI_PERT_DIST;
  ctiler(tilerFlags, 0.005f);
  std::vector<std::vector<float> > tiles = getTiles();
  ...
  for (contourIndex = 0; contourIndex < numContours; contourIndex++)
    {
      int numPoints = pointsPerContour[contourIndex];
      for (int pointIndex = 0; pointIndex < numPoints; pointIndex++)
	{
	  delete [] points[contourIndex][pointIndex];
	}
      delete [] points[contourIndex];
    }
  delete [] points;
  delete [] pointsPerContour;

  //
  // clear surface and add tiles to surface
  //
  surface.clear();

  // divide by 3 because tiles is a vector of 
  // points & there are 3 points per tile
  int tileCount = tiles.size() / 3;
  for (int tileIndex = 0; tileIndex < tileCount; ++tileIndex)
    {
      int vertex1Index = tileIndex * 3 + 0;
      int vertex2Index = tileIndex * 3 + 1;
      int vertex3Index = tileIndex * 3 + 2;

      Vector3D<double> vertex1(tiles[vertex1Index][0],
			       tiles[vertex1Index][1],
			       tiles[vertex1Index][2]);
      Vector3D<double> vertex2(tiles[vertex2Index][0],
			       tiles[vertex2Index][1],
			       tiles[vertex2Index][2]);
      Vector3D<double> vertex3(tiles[vertex3Index][0],
			       tiles[vertex3Index][1],
			       tiles[vertex3Index][2]);

      surface.addFacet(surface.addVertex(vertex1),
		       surface.addVertex(vertex2),
		       surface.addVertex(vertex3));
    }
}
...
::anastructToSurfacePowerCrust(const Anastruct &anastruct, Surface& surface)
{
  ...
  while (stillMoreCells)
    { 
      ...
      if(stillMoreCells)
	{      	 
	  ptType* vert1= Vertices->GetPoint(cell[0]);
	  ptType* vert2= Vertices->GetPoint(cell[1]);
	  ptType* vert3= Vertices->GetPoint(cell[2]);

	  surface.addFacet(
              surface.addVertex(vert1[0], vert1[1], vert1[2]),
              surface.addVertex(vert2[0], vert2[1], vert2[2]),
              surface.addVertex(vert3[0], vert3[1], vert3[2]) );
...
#endif
  if (fdes < 0 || write_anastruct(fdes, &pluncAnastruct)) //was &pluncAnastruct
    {
