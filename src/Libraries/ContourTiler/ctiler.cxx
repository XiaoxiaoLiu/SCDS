...
vector<vector<float> > outputTriangles;

//write a tile to a file descriptor, and add the tile to the
//outputTriangle list.
void write_tile(FILE *fptile,float x1,float y1,float z1,float x2,float y2,float z2,float x3,float y3,float z3) {
    //fprintf(fptile,"%f %f %f\n%f %f %f\n%f %f %f\n\n",
	    //x1,y1,z1,x2,y2,z2,x3,y3,z3);
    vector<float> triangle1;
    triangle1.push_back(x1);
    triangle1.push_back(y1);
    triangle1.push_back(z1);
    outputTriangles.push_back(triangle1);
    vector<float> triangle2;
    triangle2.push_back(x2);
    triangle2.push_back(y2);
    triangle2.push_back(z2);
    outputTriangles.push_back(triangle2);
    vector<float> triangle3;
    triangle3.push_back(x3);
    triangle3.push_back(y3);
    triangle3.push_back(z3);
    outputTriangles.push_back(triangle3);
}
...
static void init_globals()
{
	...
	for (int slice=0; slice < CTI_MAX_SLICES-1; slice++) num_contours[slice] = 0;
}
 ...
 */
    ...
    printf("Options: %scaps %scw p%f %snormals\n",
        allow_caps ? "" : "no",
        cw         ? "" : "c",
        perturb_dist,
        draw_normals ? "" : "no");
 ...
 */
    ...
    //we don't want to use files in this lib anymore.
    /*if (! (fptile = fopen("tiled.dat", "w")))   {
...
/* init record of retries */
    for (i=0, ip = &retries[0][0];
        i < static_cast<int>(sizeof(retries)/sizeof(retries[0][0]));
        i++, ip++)
    *ip = -1;
...
/* slab-by-slab summary */
    ...
    {
        for (int attempt = 0; attempt < NTRIES; attempt++)
        {
...
int setContours(int numContours, int* pointCounts,float*** contourPoints){
    ncon = numContours;
    //std::cerr << "ncon = " << ncon << endl;
    for(int contourNum = 0;contourNum < numContours;contourNum++){
        nodeCnt[contourNum] = pointCounts[contourNum];
	//std::cerr << "nodeCnt[contourNum] = " << nodeCnt[contourNum] << endl;
        for(int pointNum = 0;pointNum < nodeCnt[contourNum];pointNum++){
            surface[contourNum][pointNum][0] = contourPoints[contourNum][pointNum][0];
	    //std::cerr << "x = " << surface[contourNum][pointNum][0] ;
            surface[contourNum][pointNum][1] = contourPoints[contourNum][pointNum][1];
	    //std::cerr << "y = " << surface[contourNum][pointNum][1] ;
            surface[contourNum][pointNum][2] = contourPoints[contourNum][pointNum][2];
	    //std::cerr << "z = " << surface[contourNum][pointNum][2] << endl; 
        }
    }
	return 0;
}
