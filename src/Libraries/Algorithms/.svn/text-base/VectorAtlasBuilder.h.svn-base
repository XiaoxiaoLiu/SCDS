...
public:
  ...
  void SCDSModeOn(){SCDSMode = true;}
  void SCDSModeOff(){SCDSMode = false;}
   bool SCDSMode;//SCDS warp option: only compute the mean of the first channel. only effective for ArithmeticMean
protected:
...
//std::cout << "PP ComputeMean comp = " << this->_numComponents << " w = " << this->_weights[0] << ", " << this->_weights[1] << std::endl;
    ...
    {
		if (this->SCDSMode == true){
		    mean[0] += values[i][0] * this->_weights[i];
		}
		else{
...
// NOTE: if set previously, use number of input images here
 ...
 private:
  ...
  //In scds_warp application, where the VectorImage has four channels, the first channel  contains the iHat, 
  //and the rest of the channles are displacement vector field (hfield). When updating the average image, 
  //the displacement vector field  will be kept to be identity hfield, only the image intensity got updated.
