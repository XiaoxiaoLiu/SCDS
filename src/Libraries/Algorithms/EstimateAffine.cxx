...
std::ostream& operator<<(std::ostream& os, 
                         const EstimateAffine::EstimateAffineLevelStruct& eals)
{
...
EstimateAffine::EstimateAffine(const ImageType* fixedImage,
...
EstimateAffine::EstimateAffineLevelStruct::
...
EstimateAffine::TransformType EstimateAffine::
          ...
          OutputMode outputMode )
{
    ...
    EstimateAffine estimateAffine( fixedImage, movingImage,
                                   useIntensityWindowing, outputMode );
    estimateAffine.SetShrinkSchedule( 
        EstimateAffine::GenerateScheduleFromImage( fixedImage ));
...
EstimateAffine::ScheduleType EstimateAffine::
...
// EstimateAffine::SetShrinkSchedule
void
EstimateAffine::SetShrinkSchedule( ScheduleType shrinkSchedule )
{
...
void
EstimateAffine::SetROI( ImageRegionType roi)
{
...
void
EstimateAffine::_RunEstimateTransform( TransformGroupType transformType )
{
    ...
    _levels = std::vector< EstimateAffineLevelStruct >();
...
Vector3D<double> EstimateAffine::
...
int
EstimateAffine::_InitializeLevels()
{
    unsigned int numberOfLevels = _shrinkSchedule.getSizeX();
    _levels.clear();
    _levels.insert( _levels.end(), numberOfLevels, EstimateAffineLevelStruct() );
...
void
EstimateAffine::_InitializeImages()
{
    Timer timer;
...
void
EstimateAffine::_InitializeMovingImageGradients()
{
    Timer timer;
    if (_outputMode == VERBOSE) {
...
// automatically.
void
EstimateAffine::_ComputeGradientComponent( const ImageType* image,
                                           int direction,
...
EstimateAffine::_RunEstimateTransformLevel( 
    ...
    EstimateAffine::TransformGroupType transformType )
{
...
void
EstimateAffine::_ConvertGeneralLinearToRotation( Matrix3D<double>& matrix )
{
...
float
EstimateAffine::_StepEstimateTranslation( unsigned level )
{
    ...
    for( unsigned int zPos = roiStart[2]; zPos <= roiStop[2]; zPos++ ) {
        for( unsigned int yPos = roiStart[1]; yPos <= roiStop[1]; yPos++ ) {
            for( unsigned int xPos = roiStart[0]; xPos <= roiStop[0]; xPos++ ) {
                ...
                size_t i0, i1, i2, i3, i4, i5, i6, i7;
...
// the two images.
float
EstimateAffine::_StepEstimateAffine( unsigned level )
{
