function raw32mhd( fn_in,outDir,metaDataType,dim,origin,spacing)
...
origin= 10*fscanf(fid_in_txt,'%g %g %g',3)';
fclose(fid_in_txt);
end



WriteMhdHeader([outDir,'\',fn_base,'.mhd'],fn_base, dim,metaDataType,origin,spacing);


% Clip the header
nVals = prod(dim);
bytesExpected = nVals * 2;  % 2 bytes per short/ushort
fseek( fid_in, -bytesExpected, 'eof' );
im = fread(fid_in,nVals,decideMETADataType(metaDataType));
fclose(fid_in);

disp(['Writing file ' fn_out] );
fid_out = fopen( fn_out ,'w' );
fwrite( fid_out, im, decideMETADataType(metaDataType) );
fclose(fid_out);